...
while ( $current_packet <= $num_packets )
  set comparison_packet = $current_packet
  echo $current_packet
  while ( $comparison_packet > 1 )
    echo $comparison_packet
    set comp_packet = `expr $comparison_packet - 1`
    set org_format_first = `sed "${comparison_packet}q;d" reference_file`
    set first = (`echo $org_format_first | sed 's/,/) (/g'  | sed 's/) (/)_(/g'`)
    set org_format_second = `sed "${comp_packet}q;d" reference_file`
    ...
    process:
      ...
      while ( $ctr[$lvl] <= $num[$lvl] )
        ...
        #echo "check3"
        ###############################
        if ( ( "$first_item" =~ [0-9]* ) && ( "$second_item" =~ [0-9]* ) ) then
          ###############################
          #echo "one"
          ###############################
          ...
          @ lvl = ( $lvl + 1 )
          goto process 
        else if ( ( `echo "$first_item" | grep -o '^.' ` != "(" ) && ( `echo "$second_item" | grep -o '^.'` == "(" )  && ( "$first_item" != "" ) ) then
          ###############################
          #echo "three"
          ###############################
          ...
          @ lvl = ( $lvl + 1 )
          goto process
        else if ( ( `echo "$second_item" | grep -o '^.' ` != "(" ) && ( `echo "$first_item" | grep -o '^.'` == "(" ) && ( "$second_item" != "" ) )  then
          ###############################
          #echo "four"
          ###############################
          ...
          if ( "$first_item" == "" && "$second_item" != "" ) then
             ...
             ###############################
             echo "Right side ran out of items : Wrong order" 
             ###############################
             set flag = 0
             break
           else 
...
tac reference_file > reference_file3
set divider_packet1 = `grep -n '^((2))$' reference_file3 | sed 's/:.*//g'`
set divider_packet2 = `grep -n '^((6))$' reference_file3 | sed 's/:.*//g'`
