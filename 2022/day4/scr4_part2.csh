...
foreach line (`cat input4.txt`)
  ...
  if ( $first_pair_lower <= $second_pair_lower && $second_pair_lower <= $first_pair_higher ) then
      set flag = 1
  endif
  if ( $second_pair_lower <= $first_pair_lower && $first_pair_lower <= $second_pair_higher ) then
      set flag = 1
  endif
  if ( $flag ) then
      @ count = ( $count + 1 )
