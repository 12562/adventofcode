...
#############
  ...
  echo "CR:$current_rock WDN:$wind_dir_num_for_record `expr $max - $two` `expr $max - $three` `expr $max - $four` `expr $max - $five` `expr $max - $six` `expr $max - $seven` `expr $max - $eight`" >> find_cycle
  echo "MAX:$max" >> find_cycle
#############
