...
echo 1 > openSet 
seq $total | sed 's/.*/0/g' > cameFrom
seq $total | sed 's/.*/100000/g' | sed '1 s/0\+//g' > gScore
seq $total | sed 's/.*/100000/g' | sed '1 s/0\+//g' > fScore

while (`cat openSet | wc -l`)
    set current = 
...
#          echo "NV: $neighborV"
  ...
  set init = `cat dist | grep -n "." | awk -v col=$num_cols -v row=$num_rows 'NR%col == 0 || int((NR+col)/row) == row  {print $0}' | sort -k 2 -n -t ":" | head -n 1 | awk -F ':' '{print $1}'`
  echo $init
